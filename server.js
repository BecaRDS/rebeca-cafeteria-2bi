...
function parseCSVLine(line) {
    // Divide a linha em colunas pelo separador de vírgula
...
app.put("/administrators/:id", (req, res) => {
    ...
    if (index === -1) return res.status(404).json({ message: "Administrador não encontrado" });

    administrators[index] = { id, email, senha, tipo };
    saveAdministratorsToCSV();
    res.json(administrators[index]);
});
